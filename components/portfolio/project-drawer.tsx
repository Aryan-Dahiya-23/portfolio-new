...
export function ProjectDrawer({ activeProject, onClose }: ProjectDrawerProps) {
  return (
    <>
      ...
      <aside
        className={`fixed inset-0 z-50 h-full w-full overflow-y-auto bg-[#fffaf6] p-6 shadow-2xl transition-transform duration-300 sm:p-8 ${
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={onClose}
                  className="rounded-full border border-[var(--line)] px-4 py-2 text-sm font-medium transition hover:bg-white"
                >
                  Close
                </button>
              </div>
